...
# After Effects Pipeline Guide
  ...
  - Read once per phase immediately after the phase loads `jsonData`.
  - `ENABLE_EXTRA_OUTPUT_COMPS` remains explicitly controlled (presence of `config.pack.EXTRA_OUTPUT_COMPS` does not auto-enable it).

  Supported keys per phase:
  ...
  | Key | Type | Description |
  |-----|------|-------------|
  | `LAYER_NAME_CONFIG` | object | Layer identification map (exact/contains per group key). |
  | `TIMING_BEHAVIOR` | object | Map of group key → `timed`\|`span`\|`asIs`. |
  | `TIMING_ITEM_SELECTOR` | object | Map of group key → `{ mode, value }` selector. |
  | `SKIP_COPY_CONFIG` | object | Skip-copy rules (flags, groups, adHoc). |
  ...
  ```json
  {
    "config": {
      "pack": {
        "EXTRA_OUTPUT_COMPS": { "16x9|15s": [{ "size": "1920x1080", "media": "YouTube" }] }
      },
      "addLayers": {
        "TIMING_BEHAVIOR": { "logo": "timed", "disclaimer": "span" }
      }
    }
  }
  ```
