...
if (segments.length === 0) {
  ...
  if (!active) {
    ...
    // Ensure first sampled frame at start of fade is non-zero (or 100 if effectively instant)
    var fd = thisComp.frameDuration;
    if (t < sT + fin) {
      if (FADE_IN <= 0 || fin <= fd) {
        100; // treat as instant-on at segment start
      } else {
        var tAdj = Math.min(t + fd * 0.5, sT + fin); // nudge forward within fade window
        linear(tAdj, sT, sT + fin, OPAC_IN, 100);
      }
    } else {
      100;
    }
