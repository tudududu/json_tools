...
SEMVER_RE = re.compile(
    r"^(?:\[)?v?(?P<ver>[0-9]+\.[0-9]+\.[0-9]+(?:[-+][A-Za-z0-9.]+)?)(?:\])?"
)
...
def git_run(args: List[str]) -> Tuple[int, str]:
    try:
        res = subprocess.run(
            args, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False, text=True
        )
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        description="Auto-bump CHANGELOG.md and optionally create git commit/tag"
    )
    ...
    g.add_argument(
        ...
        default="patch",
        help="Semantic version component to increment (ignored if --set provided)",
    )
    g.add_argument(
        "--set", dest="set_version", help="Explicit version to set (overrides --part)"
    )
    p.add_argument("--pre", help="Pre-release label to append (e.g. rc1)")
    p.add_argument(
        "--date", help="Override release date (YYYY-MM-DD). Default: today UTC"
    )
    p.add_argument(
        "--dry-run",
        action="store_true",
        help="Print result to stdout; do not modify file",
    )
    p.add_argument(
        "--commit",
        action="store_true",
        help="Create a git commit for the bumped CHANGELOG",
    )
    p.add_argument(
        "--tag",
        action="store_true",
        help="Create git tag v<version> after bump (implies --commit)",
    )
    p.add_argument(
        "--force-tag",
        action="store_true",
        help="Overwrite existing tag if it already exists",
    )
    p.add_argument(
        "--no-placeholder",
        action="store_true",
        help="Do not add placeholder section when no unreleased bullets",
    )
    ...
    if not current and not args.set_version:
        print(
            "ERROR: Could not detect current version from first heading and no --set provided.",
            file=sys.stderr,
        )
        return 2
    ...
    if (args.commit or args.tag) and not args.dry_run:
        ...
        # Commit
        msg = (
            f"chore: release {new_version}"
            if not unreleased_bullets
            else f"chore: bump to {new_version}"
        )
        ...
        if args.tag:
            ...
            if code == 0 and not args.force_tag:
                print(
                    f"ERROR: tag {tag_name} already exists (use --force-tag to overwrite)"
                )
                return 3
