...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Inspect JSON outputs for selected metadata keys"
    )
    ap.add_argument(
        "paths", nargs="+", help="JSON files, directories, or glob patterns"
    )
    ap.add_argument(
        "--keys",
        default="disclaimer_flag,subtitle_flag,jobNumber",
        help="Comma list of metadata keys to extract (default: disclaimer_flag,subtitle_flag,jobNumber)",
    )
    ap.add_argument(
        "--show-missing",
        action="store_true",
        help="Include keys with value <MISSING> when absent",
    )
    ap.add_argument(
        "--per-video",
        action="store_true",
        help="List keys per video instead of only metadataGlobal",
    )
