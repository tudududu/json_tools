...
def convert_rows(
    rows: Iterable[dict], trim: bool = True
...
def read_csv(
    path: str, delimiter: str = ";", xlsx_sheet: str | None = None
) -> List[dict]:
    ...
    if ext in (".xlsx", ".xlsm", ".xltx", ".xltm"):
        ...
        try:
            if xlsx_sheet:
                ...
                default_sheet_name = (
                    "media" if "media" in wb.sheetnames else wb.sheetnames[0]
                )
                ws = wb[default_sheet_name]

            if delimiter and delimiter != ";":
                print(
                    "Warning: --delimiter is ignored for XLSX input.", file=sys.stderr
                )
            ...
            for row in rows_iter:
                ...
                item = {
                    fieldnames[i]: (row_vals[i] if i < len(row_vals) else "")
    ...
    raise ValueError(
        "Missing required headers; tried delimiters: " + ", ".join(delims_to_try)
    )
...
def main() -> None:
    ...
    p.add_argument(
        "--xlsx-sheet",
        default=None,
        help="XLSX only: sheet name to read (default: 'media' if present, otherwise first sheet)",
    )
    ...
    grp.add_argument(
        ...
        dest="trim",
        action="store_true",
        help="Trim whitespace on all fields (default)",
    )
    grp.add_argument(
        "--no-trim",
        dest="trim",
        action="store_false",
        help="Disable whitespace trimming",
    )
    p.set_defaults(trim=True)
    p.add_argument(
        "--dry-run",
        action="store_true",
        help="Parse only and print summary; do not write JSON",
    )
    p.add_argument(
        "--compact", action="store_true", help="Write JSON with inline array items"
    )
    # Split-by-country/language options
    p.add_argument(
        "--split-by-country",
        action="store_true",
        help="Split outputs per Country/Language columns",
    )
    p.add_argument(
        "--country-col",
        default="Country",
        help="Column name used for country grouping (default 'Country')",
    )
    p.add_argument(
        "--language-col",
        default="Language",
        help="Column name used for language grouping (default 'Language')",
    )
    p.add_argument(
        "--output-pattern",
    ...
    if args.split_by_country:
        groups = group_by_country_language(
            rows,
