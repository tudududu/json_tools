...
CONVERTER = os.path.join(PROJECT, "python", "json_converter.py")
...
class LogoAnimFlagTests(unittest.TestCase):
        ...
        try:
            ...
            v = next(
                (
                    ...
                    if v.get("metadata", {}).get("duration") in ("6", 6)
                ),
                None,
            )
    ...
    def test_target_duration_column_preferred(self):
        ...
        try:
            ...
            self.assertNotIn(
                "deprecated",
                res.stderr.lower(),
                "Should not warn when target_duration is used",
            )
            deu_path = out_pattern.replace("{country}", "DEU")
            data = load_json(deu_path)
            mapping = data.get("metadataGlobal", {}).get("logo_anim_flag", {})
            ...
            v = next(
                (
                    v
                    for v in data.get("videos", [])
                    if v.get("metadata", {}).get("duration") in ("30", 30)
                ),
                None,
            )
            self.assertIsNotNone(v)
    ...
    def test_legacy_country_scope_duration_warns_once(self):
        ...
        try:
            ...
            self.assertEqual(
                lowered.count("deprecated"),
                1,
                "Legacy warning should be emitted once per run",
            )
        finally:
            shutil.rmtree(tmpdir, ignore_errors=True)
    ...
    def test_overview_present_and_per_video_injection(self):
        ...
        try:
            ...
            self.assertIn(
                "logo_anim_flag", mg, "Overview mapping missing in metadataGlobal"
            )
            ...
            self.assertTrue(
                expected_any, "No expected duration keys present in overview"
            )
            ...
            target = next(
                (
                    v
                    for v in videos
                    if v.get("metadata", {}).get("duration") in (120, "120")
                ),
                None,
            )
            ...
            if flag_val != map_val:
                self.assertIsInstance(
                    map_val,
                    str,
                    "Unexpected nested mapping retained while values differ",
                )
        finally:
            shutil.rmtree(tmpdir, ignore_errors=True)
    ...
    def test_overview_removed_with_flag(self):
        ...
        try:
            ...
            self.assertNotIn(
                "logo_anim_flag",
                mg,
                "Overview should be removed with --no-logo-anim-overview",
            )
            videos = data.get("videos", [])
            target = next(
                (
            ...
            self.assertIn(
                "logo_anim_flag",
                target["metadata"],
                "Per-video flag missing when overview disabled",
            )
        finally:
            shutil.rmtree(tmpdir, ignore_errors=True)
    ...
    def test_trimmed_overview_no_nested_objects(self):
        ...
        try:
            ...
            for dur, val in mapping.items():
                self.assertFalse(
                    isinstance(val, dict),
                    f"Duration {dur} still has nested object after trimming",
                )
        finally:
            shutil.rmtree(tmpdir, ignore_errors=True)
    ...
    def test_meta_local_logo_anim_override(self):
        ...
        try:
            ...
            self.assertTrue(
                overview and overview.get("30") in ("Y", "N"), "Overview missing 30 key"
            )
            v = next(
                (
                    v
            ...
            self.assertEqual(
                per_video_value,
                "N",
                f"Expected per-video override N, got {per_video_value}",
            )
            if overview.get("30") == "Y":
                self.assertNotEqual(
                    per_video_value,
                    overview.get("30"),
                    "Override did not supersede mapping",
                )
        finally:
            shutil.rmtree(tmpdir, ignore_errors=True)
        ...
        try:
            ...
            v = next(
                (
                    v
                    for v in data.get("videos", [])
                    if v.get("metadata", {}).get("duration") in ("45", 45)
                ),
                None,
            )
            ...
            self.assertNotIn(
                "logo_anim_flag",
                v["metadata"],
                "Unexpected logo_anim_flag for unmapped duration",
            )
        finally:
            shutil.rmtree(tmpdir, ignore_errors=True)
        ...
        try:
            ...
            v = next(
                (
                    v
                    for v in data.get("videos", [])
                    if v.get("metadata", {}).get("duration") in ("60", 60)
                ),
                None,
            )
            self.assertIsNotNone(v)
