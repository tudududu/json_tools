...
class LogPickerLayersSummaryTests(unittest.TestCase):
    def test_layers_short_summary(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn(
                "a.log: Counts => layersAddedTotal=10 ; Timing (s) => addLayers=2.5",
                txt,
            )
            # b.log missing Counts value, has Timing value
            self.assertIn(
                "b.log: Counts => layersAddedTotal=- ; Timing (s) => addLayers=7.5", txt
            )
            # c.log missing both specific keys -> dashes
            self.assertIn(
                "c.log: Counts => layersAddedTotal=- ; Timing (s) => addLayers=-", txt
            )
